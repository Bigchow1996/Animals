...
$animals_type = $_GET['type']?$_GET['type']:1; 
switch ($animals_type){
    case '哺乳类':
        $name = "哺乳动物 Mammal";
        break;
...
$sql = "select * from animals where animaltype = '{$animals_type}'";
$result=mysql_query($sql);
$totalRows_animals=mysql_num_rows($result);
$sumPage_animals = ceil($totalRows_animals/$pageSize);
$query_animals = "select * from animals where animaltype = '{$animals_type}'  order by id desc limit {$offset_animals},{$pageSize}";
