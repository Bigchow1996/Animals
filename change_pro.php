...
$link = connect3();
$pro_id = $_GET['id_pro']?$_GET['id_pro']:1;
$_SESSION['proid'] = $pro_id;
$page_say = $_GET['page_say']?$_GET['page_say']:1;
$pageSize = 5;
$offset_say = ($page_say - 1)*5;
$sql = "select * from say_protection where userid = {$pro_id}";
...
$query_pro = "select * from protection where id = {$pro_id}";

$rows_pro = fetchAll($link,$query_pro);

$query = "select * from say_protection where userid = {$pro_id} order by id desc limit {$offset_say},{$pageSize}";
        		...
        		<div class="row">
			      ...
			      <div class="col-md-12">
			       <div class='panel panel-default'>
        		    <div class='panel-heading'>修改数据(动物保护)</div>
            	    <div class='changeCon panel-body'>
            	       <form action='change_data/change_pro.php' method='post' target='pro' class='form-horizontal' role='form'>
                           <div class='form-group'>
    			       ...
    			       <form action='change_data/change_pro.php' method='post' target='pro' class='form-horizontal' role='form'>
    					   <div class='form-group'>
                    	      ...
                    	      <div class='col-sm-9'>
                    		    <input type='text' name='addr' value='<?php echo $admin['addr']?>' class='form-control'>
    					      </div>
    					      <div class='col-sm-2'>
        						 <button type='submit' class='btn btn-primary'>修改</button>
        					   </div>
                    ...
                    <nav>
    					...
    					<?php
    					   for($i = 1;$i<=$sumPage_say;$i++){
    					       echo "<li><a href='change_pro.php?page_say={$i}&pro_id={$pro_id}'>$i</a></li>";
            ...
            <script>
            	  $(function(){
        		        $(".delete").click(function(){
            		        ...
            		        var msg = "您真的确定要删除吗？\n\n请确认！";
            		        if (confirm(msg)==true){
            		        	var url = "delect_data/delect_pro.php?say_id_pro="+id;
                		        $.get(url);
